...
@ApiMethod  --> in method
  ...
  commentInReturnExampleWithLevel use level to handle comment in field, default false
    ...
    but, if use @JsonPropertyOrder to change field sort, or the field is not a standard hump name will be a problem

    if set to true, the level in the field will be used to process the comment,
    but, there will be problems when encountering a complex generic
