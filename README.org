...
public class ApiInfoConfig {
 ...
    public DocumentCopyright apiCopyright() {
    ...
                // .setIgnoreUrlSet(Sets.newHashSet("/user*", "/product/info|post"))
                ...
                //         DocumentParam.buildToken("x-token", "oauth info", "abc-xyz", ParamType.Header).setHasTextarea("1"),
                ...
                // //   If true, please ensure that the global response description and global token of all items are consistent.
                // //      Duplication and appending together (current practice) will cause documentation errors.
                // //   If false, multiple documents will be requested on the page, please ensure that all projects have cors turned on,
                // //     otherwise it will be inaccessible due to cross-domain issues
                // .setProjectMerge(true)

                // // use when you want to collect multiple documents in one place,
                // // ensure that the global response description and global token of all projects are consistent.
                // // if not, duplicate and attaching them together(current practice) may cause documentation errors.
...
Run and Request ~http://127.0.0.1:8080/api-info.html~, see[[https://github.com/liuanxin/api-document-example-en][example project(spring boot 2)]]
...
@ApiGroup --> in class or method. use the method if all of that
  ...
  index     --> can not be set, more forward when smaller("index" first, "module info" second -> with sort), if set multi module and different index, use the smaller
...
@ApiParam --> in param or field
  ...
  dataType    --> if type was custom can use, can not be set(for example: enum, but param type was be int). can be: int、long、float、double、date、phone、email、url、ipv4
...
@ApiModel --> Combined with the annotations of the @ApiParam and @ApiReturn, Please do not abuse. You should try to use the first two.
  ...
  dataType  --> (return type or param name)'s type, can not be set, use with customize: int、long、float、double、date、phone、email、url、ipv4
