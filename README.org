...
如果方法上的返回结果是 List、Set 或 Map 会以 ArrayList、HashSet 及 HashMap 进行处理.

PS: 方法上的返回类型如果是 ResponseEntity 这一类 没有默认无参构造 的类将无法处理(可以参考 [[https://github.com/liuanxin/api-document-example][example]] 的处理).
另外, 当返回值中有数组时也无法实例, 请使用 List 替换
