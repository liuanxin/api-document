...
public class ApiInfoConfig {
 ...
                    // .setIgnoreUrlSet(Sets.newHashSet("/user*", "/product/info|post"))
                    // // generated on each api's param, see the ApiToken annotation below
                    // .setGlobalToken(DocumentParam.buildToken("name", "desc", "example", ParamType.Header))
...
@ApiToken --> in class or method
  value     --> if set false, the api of the current method or class does not need to pass token information. When the global setting and some api are not needed, use @ApiToken(false)
  name      --> can not set
  desc      --> can not set
  example   --> can not set
