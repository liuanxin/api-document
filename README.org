...
维护过项目的人应该都有体会, 如果接口文档是单独编写的(org-mode、markdown、rap 等), 随着项目周期的推进, 接口文档和真实代码之间的差距会越来越远.

基于 swagger 的一些细节不那么让人如意(比如无法处理 map, 响应说明需要切屏才能查看)而出现了此文档收集应用.
...
public class ApiInfoConfig {
 ...
                    // 如果设置成 true 则接口会返回空(生产环境时开启), 默认是 false
                    // .setOnline(true)
...
可以参考 [[https://github.com/liuanxin/api-document-example][example]] 的全局处理方式
