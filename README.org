...
In addition, when has array of non-base data types in the return field, please use List<Type> to replace
...
@ApiMethod  --> in method
  ...
  commentInReturnExampleWithLevel use level to handle comment in field, default false
    if set to false, will use index corresponding to the field to process the comment,
    but, if use @JsonPropertyOrder to change field sort, or the field is not a standard hump name, there will be problems

    if set to true, the hierarchical relationship corresponding to the field will be used to process the comment,
    but, when encountering complex generic structures, or the field is not a standard hump name, there will be problems
