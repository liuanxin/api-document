...
[[https://github.com/liuanxin/api-document-example][示例项目(spring boot 4)]]
...
PS: 可以参考 [[https://github.com/liuanxin/api-document-example][示例项目(spring boot 4)]] 的全局处理.
...
项目使用 ~springboot2~、~springboot3~、~springboot4~ 三个分支维护对应的 Spring Boot 版本. 为避免一次版本发布占用三次 Maven Central Release Count, 三个分支必须合并成一个 deployment 发布.

每个分支先执行测试, 再只生成 bundle, 不直接上传:
#+BEGIN_SRC shell
mvn clean deploy -P release -DskipPublishing=true
#+END_SRC

每次构建完成后, 将 ~target/central-publishing/central-bundle.zip~ 解压到同一个空目录. 三个分支的版本目录不同, 可以直接合并. 最后从该目录重新生成一个 zip, 通过 Central Portal 或 Publisher API 一次上传并发布.

例如, 每个分支构建后分别将 bundle 保存为 ~/tmp/api-document-springboot2.zip~、~/tmp/api-document-springboot3.zip~、~/tmp/api-document-springboot4.zip~, 再执行:
#+BEGIN_SRC shell
...
最终只上传 ~$release_dir/api-document-release.zip~. 不要分别上传三个原始 bundle.

Publisher API 上传时使用 ~publishingType=AUTOMATIC~, 发布完成后确认 deployment 状态为 ~PUBLISHED~. Maven Central 当前额度和使用量以 [[https://central.sonatype.com/publishing/usage][Usage Center]] 为准.
