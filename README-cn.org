...
@ApiMethod  标注在方法上
  ...
  commentInReturnExampleWithLevel 使用层级关系来处理示例中的字段注释, 默认是 false
    为 false 将使用字段的所在位置来处理说明, 但是 当使用 @JsonPropertyOrder 更改字段顺序 或 字段不是标准的驼峰命名 将会有问题
    为 true  将使用字段的层级关系来处理说明, 但是 当遇到复杂的泛型结构 或 字段不是标准的驼峰命名 将会有问题
