...
public class ExampleController {
    @ApiMethod("用户列表")
    @GetMapping("/example")
    public JsonResult<DemoVo> example(@ApiParam("类型") String type, Page page) {
        return JsonResult.success("示例接口", new DemoVo());
    }
}
...
运行后浏览器 ~http://127.0.0.1:8080/api-info.html~ 即可查看到效果, 见[[https://github.com/liuanxin/api-document-example][示例项目(spring boot 2)]]
